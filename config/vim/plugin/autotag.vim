...
class AutoTag:
   ...
   def updateTagsFile(self, tagsFile, sources):
      ...
      if self.tags_file:
         cmd = "%s -R --c++-kinds=+p --fields=+iaS --extra=+q+f --tag-relative=yes --language-force=C++ -f %s -a " % (self.ctags_cmd, self.tags_file)
      else:
         cmd = "%s -R --c++-kinds=+p --fields=+iaS --extra=+q+f --tag-relative=yes --language-force=C++ -a " % (self.ctags_cmd,)
