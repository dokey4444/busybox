...
void md5(unsigned char *input_buf, unsigned int buf_len, unsigned char *output_buf)
{
	MD5_CTX context;
	MD5Init(&context);
	MD5Update(&context, input_buf, buf_len);
	MD5Final(output_buf, &context);
}

uint32_t md5_32(unsigned char *input_buf, unsigned int buf_len)
{
	unsigned char md5_buf[16];
	md5(input_buf, buf_len, md5_buf);
// :WARNING:11/09/2013 05:23:49 PM:elwin: dereferencing type-punned pointer will break strict-aliasing rules [-Wstrict-aliasing]
	return *(uint32_t*)md5_buf;
}
